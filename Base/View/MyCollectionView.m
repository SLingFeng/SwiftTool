...
-(NSAttributedString *)titleForEmptyDataSet:(UIScrollView *)scrollView {
    ...
    switch (self.cState) {
        case MyCollectionViewStateNormal:
            return nil;
            break;
        case MyCollectionViewStateNoData: {
                text = @"暂无数据";
            }
            break;
        case MyCollectionViewStateFailedLoad: {
                text = @"加载失败";
            }
            break;
        case MyCollectionViewStateError: {
                text = @"加载出错";
            }
            break;
        case MyCollectionViewStateUnknownError: {
                text = @"未知错误";
            }
            break;
...
- (NSAttributedString *)buttonTitleForEmptyDataSet:(UIScrollView *)scrollView forState:(UIControlState)state {
    ...
    NSDictionary * attributes = @{NSFontAttributeName : [SLFCommonTools pxFont:32], NSForegroundColorAttributeName : [SLFCommonTools colorHex:@"666666"]};
...
- (void)setCState:(MyCollectionViewState)cState {
    _cState = cState;
