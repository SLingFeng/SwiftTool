 ...
 */
-(void)commonInit{
    self.BD = BorderDirectionAllCorners;
...
- (void)drawRect:(CGRect)rect {
    ...
    [LFRView setVariableRoundedBorder:rect view:self
                                                     BD:self.BD
...
+(void)setVariableRoundedBorder:(CGRect)rect view:(UIView *)view  BD:(BorderDirection)BD corners:(UIRectCorner)corners radius:(CGFloat)radius borderWidth:(CGFloat)borderWidth borderColor:(UIColor *)borderColor {
    ...
    if (corners != 0) {
        [self setRounded:rect view:view corners:corners radius:radius];
    }
    if (BD != 0) {
 ...
 *  @param corners 需要设置圆角的方向
 */
+(void)setRounded:(CGRect)rect view:(UIView *)view corners:(UIRectCorner)corners radius:(CGFloat)radius {
