...
//            let nameX =  (type == ChatType.someone) ? avatarX + avatarImage.frame.width + 20 : kScreenW - (kScreenW - 220)
        ...
        if (type == ChatType.someone)
        {
            self.bubbleImage.image = UIImage(named:("home_chat_white"))//!.stretchableImage(withLeftCapWidth: 21,topCapHeight:25)
            
        }
        else {
            self.bubbleImage.image = UIImage(named:"home_chat_green")//.stretchableImage(withLeftCapWidth: 15, topCapHeight:25)
        }
        ...
        //如果是图片不显示泡泡
        if self.customView.isKind(of: UIImageView.self) {
